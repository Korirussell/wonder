...
class AbletonMCP(ControlSurface):
    ...
    def _delete_clip(self, track_index, clip_index):
        """Delete (clear) a clip from a clip slot"""
        try:
            if track_index < 0 or track_index >= len(self._song.tracks):
                raise IndexError("Track index out of range")
            track = self._song.tracks[track_index]
            ...
            clip_slot = track.clip_slots[clip_index]
            if not clip_slot.has_clip:
                return {"deleted": False, "reason": "No clip in slot"}
    ...
    def _search_browser(self, query, category="all"):
        """Search the browser for items matching a query string"""
        try:
            app = self.application()
            ...
            def search_item(item, depth=0):
                ...
                try:
                    ...
                    if hasattr(item, 'children') and depth < 7:
                        for child in item.children:
                            search_item(child, depth + 1)
                except Exception:
                    pass

            browser = app.browser
            roots = []
            if category in ("all", "instruments") and hasattr(browser, 'instruments'):
                roots.append(browser.instruments)
            if category in ("all", "sounds") and hasattr(browser, 'sounds'):
                roots.append(browser.sounds)
            if category in ("all", "drums") and hasattr(browser, 'drums'):
                roots.append(browser.drums)
            if category in ("all", "audio_effects") and hasattr(browser, 'audio_effects'):
                roots.append(browser.audio_effects)
            if category in ("all", "midi_effects") and hasattr(browser, 'midi_effects'):
                roots.append(browser.midi_effects)
            ...
            return {
                ...
                "category": category,
                "count": len(matches),
                "items": matches[:20],  # Cap at 20 results
            }
        except Exception as e:
