...
def _detect_key(chroma: np.ndarray) -> str:
    ...
    for root in range(12):
        ...
        major_profile = _KS_MAJOR - _KS_MAJOR.mean()
        minor_profile = _KS_MINOR - _KS_MINOR.mean()

        major_r = float(np.corrcoef(rotated, major_profile)[0, 1])
        minor_r = float(np.corrcoef(rotated, minor_profile)[0, 1])
