...
ALLOWED_COMMANDS = {
    ...
    "get_clip_launch_mode",
    "set_clip_launch_mode",
    "get_clip_launch_quantization",
    "set_clip_launch_quantization",
    "get_clip_follow_action",
    "set_clip_follow_action",
    # Track state
    "get_track_playing_slot_index",
    "get_track_fired_slot_index",
...
class AbletonConnection:
    ...
    def send_command(self, command_type: str, params: dict = None) -> dict:
            ...
            for attempt in range(self._max_retries + 1):
                if not self.connect():
                    if attempt < self._max_retries:
                        logger.warning(
                            f"Connection failed, retrying ({attempt + 1}/{self._max_retries})"
                        )
                        continue
                    raise HTTPException(
                        status_code=503,
                ...
                try:
                    ...
                    if attempt < self._max_retries:
                        logger.warning(
                            f"Command failed, retrying ({attempt + 1}/{self._max_retries}): {last_error}"
                        )
                except Exception as e:
                    last_error = str(e)
                    self._reconnect()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Command failed after {self._max_retries} retries: {last_error}",
        )
...
class QuantizeRequest(BaseModel):
    grid: float = (
        0.25  # Grid size in beats (0.25 = 16th notes, 0.5 = 8th, 1.0 = quarter)
    )
...
def get_all_tracks(
    limit: int = Query(
        None, ge=1, le=1000, description="Maximum number of tracks to return"
    ),
...
def get_track_info(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX, description="Track index"),
):
...
def create_midi_track(req: TrackCreateRequest):
    return ableton.send_command(
        "create_midi_track", {"index": req.index, "name": req.name}
    )
...
def create_audio_track(req: TrackCreateRequest):
    return ableton.send_command(
        "create_audio_track", {"index": req.index, "name": req.name}
    )
...
def set_track_name(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX), req: TrackNameRequest = None
):
    return ableton.send_command(
        "set_track_name", {"track_index": track_index, "name": req.name}
    )
...
def set_track_color(
    ...
    req: TrackColorRequest = None,
):
    return ableton.send_command(
        "set_track_color", {"track_index": track_index, "color": req.color}
    )


@app.put("/api/tracks/{track_index}/mute")
def set_track_mute(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX), req: TrackBoolRequest = None
):
    return ableton.send_command(
        "set_track_mute", {"track_index": track_index, "mute": req.value}
    )
...
def set_track_solo(
    ...
    return ableton.send_command(
        "set_track_solo", {"track_index": track_index, "solo": req.value}
    )
...
def set_track_arm(
    ...
    return ableton.send_command(
        "set_track_arm", {"track_index": track_index, "arm": req.value}
    )
...
def set_track_volume(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX),
    req: TrackVolumeRequest = None,
):
    return ableton.send_command(
        "set_track_volume", {"track_index": track_index, "volume": req.volume}
    )


@app.put("/api/tracks/{track_index}/pan")
def set_track_pan(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX), req: TrackPanRequest = None
):
    return ableton.send_command(
        "set_track_pan", {"track_index": track_index, "pan": req.pan}
    )


# Track Monitoring
class MonitoringRequest(BaseModel):
    monitoring: int = Field(
        ..., ge=0, le=2, description="Monitoring state (0=In, 1=Auto, 2=Off)"
    )
...
def set_track_monitoring(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX),
    req: MonitoringRequest = None,
):
    """Set track monitoring state (0=In, 1=Auto, 2=Off)"""
    return ableton.send_command(
        "set_track_monitoring",
        {"track_index": track_index, "monitoring": req.monitoring},
    )


# Group Tracks
class GroupTrackRequest(BaseModel):
    track_indices: List[int] = Field(
        ..., min_length=1, description="List of track indices to group"
    )
...
def create_group_track(req: GroupTrackRequest):
    ...
    return ableton.send_command(
        "create_group_track", {"track_indices": req.track_indices}
    )
...
def set_clip_name(
    ...
    return ableton.send_command(
        "set_clip_name",
        {"track_index": track_index, "clip_index": clip_index, "name": req.name},
    )
...
def set_clip_color(
    ...
    return ableton.send_command(
        "set_clip_color",
        {"track_index": track_index, "clip_index": clip_index, "color": req.color},
    )
...
def set_clip_gain(
    ...
    return ableton.send_command(
        "set_clip_gain",
        {"track_index": track_index, "clip_index": clip_index, "gain": req.gain},
    )
...
def set_clip_pitch(
    ...
    return ableton.send_command(
        "set_clip_pitch",
        {"track_index": track_index, "clip_index": clip_index, "pitch": req.pitch},
    )
...
class AutomationRequest(BaseModel):
    points: List[Dict[str, float]] = Field(
        ...,
        max_length=10000,
        description="List of automation points with 'time' and 'value' keys",
    )
...
class WarpModeRequest(BaseModel):
    warp_mode: int = Field(
        ...
        ge=0,
        le=6,
        description="Warp mode (0=Beats, 1=Tones, 2=Texture, 3=Re-Pitch, 4=Complex, 5=Rex, 6=Complex Pro)",
    )
...
def get_all_scenes(
    limit: int = Query(
        None, ge=1, le=1000, description="Maximum number of scenes to return"
    ),
...
def set_scene_name(
    scene_index: int = Path(..., ge=0, le=MAX_SCENE_INDEX), req: SceneNameRequest = None
):
    return ableton.send_command(
        "set_scene_name", {"scene_index": scene_index, "name": req.name}
    )
...
def set_scene_color(
    scene_index: int = Path(..., ge=0, le=MAX_SCENE_INDEX),
    req: SceneColorRequest = None,
):
    return ableton.send_command(
        "set_scene_color", {"scene_index": scene_index, "color": req.color}
    )
...
def get_device_parameters(
    ...
    limit: int = Query(
        None, ge=1, le=1000, description="Maximum number of parameters to return"
    ),
...
def get_device_by_name(
    track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX), device_name: str = Path(...)
):
...
@app.post(
    "/api/tracks/{track_index}/devices/{device_index}/chains/{chain_index}/select"
)
...
def set_send_level(
    ...
    return ableton.send_command(
        "set_send_level",
        {"track_index": track_index, "send_index": send_index, "level": req.level},
    )


@app.put("/api/returns/{return_index}/volume")
def set_return_volume(
    return_index: int = Path(..., ge=0, le=MAX_SEND_INDEX),
    req: ReturnVolumeRequest = None,
):
    return ableton.send_command(
        "set_return_volume", {"return_index": return_index, "volume": req.volume}
    )


@app.put("/api/returns/{return_index}/pan")
def set_return_pan(
    return_index: int = Path(..., ge=0, le=MAX_SEND_INDEX), req: ReturnPanRequest = None
):
    """Set return track pan (-1.0 to 1.0)"""
    return ableton.send_command(
        "set_return_pan", {"return_index": return_index, "pan": req.pan}
    )
...
def get_scale_notes(root: str, scale_type: str, octave: int = 4):
    ...
    return ableton.send_command(
        "get_scale_notes", {"root": root_midi, "scale_type": scale_type}
    )
...
def humanize_timing(
    ...
    return ableton.send_command(
        "humanize_clip_timing",
        {"track_index": track_index, "clip_index": clip_index, "amount": req.amount},
    )
...
def search_browser(req: BrowserSearchRequest):
    ...
    return ableton.send_command(
        "search_browser", {"query": req.query, "category": req.category}
    )
...
def load_item_to_track(req: LoadItemToTrackRequest):
    ...
    return ableton.send_command(
        "load_instrument_or_effect", {"track_index": req.track_index, "uri": req.uri}
    )
...
def load_item_to_return(req: LoadItemToReturnRequest):
    ...
    return ableton.send_command(
        "load_browser_item_to_return",
        {"return_index": req.return_index, "item_uri": req.uri},
    )
...
class BrowserPathRequest(BaseModel):
    path: str = Field(
        ...,
        max_length=1024,
        description="Browser path like 'Sounds/Drums' or 'Audio Effects/EQ'",
    )


@app.get("/api/browser/items")
def get_browser_items_at_path(
    path: str = Query(
        ...,
        max_length=1024,
        description="Browser path like 'Sounds/Drums' or 'Audio Effects/EQ'",
...
def set_arrangement_loop(loop_start: float, loop_length: float, loop_on: bool = True):
    ...
    return ableton.send_command(
        "set_arrangement_loop",
        {"loop_start": loop_start, "loop_length": loop_length, "loop_on": loop_on},
    )
...
def get_track_output_routing(track_index: int = Path(..., ge=0, le=MAX_TRACK_INDEX)):
    ...
    return ableton.send_command(
        "get_track_output_routing", {"track_index": track_index}
    )
...
COMMAND_PARAM_SCHEMAS = {
    ...
    # Track commands
    "create_midi_track": {
        "index": {"type": int, "min": -1, "max": MAX_TRACK_INDEX, "optional": True},
        "name": {"type": str, "max_length": 256, "optional": True},
    },
    "create_audio_track": {
        "index": {"type": int, "min": -1, "max": MAX_TRACK_INDEX, "optional": True},
        "name": {"type": str, "max_length": 256, "optional": True},
    ...
    "set_track_name": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "name": {"type": str, "max_length": 256},
    },
    "set_track_mute": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "mute": {"type": bool},
    ...
    "set_track_volume": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "volume": {"type": float, "min": 0, "max": 1},
    },
    "set_track_pan": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "pan": {"type": float, "min": -1, "max": 1},
    },
    "set_track_color": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "color": {"type": int, "min": 0, "max": 69},
    },
    "select_track": {"track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX}},
    # Clip commands
    "create_clip": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "length": {"type": float, "min": 0.01, "max": 1024, "optional": True},
        "name": {"type": str, "max_length": 256, "optional": True},
    },
    "delete_clip": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
    ...
    "duplicate_clip": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "target_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    },
    "get_clip_info": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "set_clip_loop": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "loop_start": {"type": float, "min": 0, "max": 100000, "optional": True},
        "loop_end": {"type": float, "min": 0, "max": 100000, "optional": True},
        "looping": {"type": bool, "optional": True},
    },
    "select_clip": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "add_notes_to_clip": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "notes": {"type": list, "max_length": 10000},
    },
    "remove_notes": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "pitch": {"type": int, "min": -1, "max": 127, "optional": True},
        "start_time": {"type": float, "min": 0, "max": 100000},
        "end_time": {"type": float, "min": 0, "max": 100000},
    },
    "remove_all_notes": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "transpose_notes": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "semitones": {"type": int, "min": -127, "max": 127},
    },
    # Scene commands
    "get_all_scenes": {},
    "create_scene": {
        "index": {"type": int, "min": -1, "max": MAX_SCENE_INDEX, "optional": True},
        "name": {"type": str, "max_length": 256, "optional": True},
    },
    ...
    "set_scene_name": {
        "scene_index": {"type": int, "min": 0, "max": MAX_SCENE_INDEX},
        "name": {"type": str, "max_length": 256},
    },
    ...
    "get_device_parameters": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "device_index": {"type": int, "min": 0, "max": MAX_DEVICE_INDEX},
    },
    "set_device_parameter": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "device_index": {"type": int, "min": 0, "max": MAX_DEVICE_INDEX},
        "parameter_index": {"type": int, "min": 0, "max": MAX_PARAMETER_INDEX},
    ...
    "set_track_input_routing": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "routing_type": {"type": str, "max_length": 64},
        "routing_channel": {"type": str, "max_length": 64, "optional": True},
    },
    "set_track_output_routing": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "routing_type": {"type": str, "max_length": 64},
    ...
    "set_track_monitoring": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "monitoring": {"type": int, "min": 0, "max": 2},
    },
    "get_track_monitoring": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX}
    },
    ...
    "get_return_tracks": {},
    "get_return_track_info": {
        "return_index": {"type": int, "min": 0, "max": MAX_SEND_INDEX}
    },
    "get_send_level": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "send_index": {"type": int, "min": 0, "max": MAX_SEND_INDEX},
    },
    "set_send_level": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "send_index": {"type": int, "min": 0, "max": MAX_SEND_INDEX},
        "level": {"type": float, "min": 0, "max": 1},
    },
    "set_return_volume": {
        "return_index": {"type": int, "min": 0, "max": MAX_SEND_INDEX},
        "volume": {"type": float, "min": 0, "max": 1},
    ...
    "get_arrangement_length": {},
    "set_arrangement_loop": {
        "loop_start": {"type": float, "min": 0, "max": 100000},
        "loop_length": {"type": float, "min": 0.01, "max": 100000},
        "loop_on": {"type": bool, "optional": True},
    },
    "jump_to_time": {"time": {"type": float, "min": 0, "max": 100000}},
    "get_locators": {},
    "create_locator": {
        "time": {"type": float, "min": 0, "max": 100000},
        "name": {"type": str, "max_length": 256, "optional": True},
    },
    ...
    # AI helpers
    "get_scale_notes": {
        "root": {"type": int, "min": 0, "max": 127},
        "scale_type": {"type": str, "max_length": 32},
    },
    "quantize_clip_notes": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "grid": {"type": float, "min": 0.01, "max": 16, "optional": True},
        "strength": {"type": float, "min": 0, "max": 1, "optional": True},
    },
    "humanize_clip_timing": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "generate_drum_pattern": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "style": {"type": str, "max_length": 32, "optional": True},
        "length": {"type": float, "min": 0.5, "max": 64, "optional": True},
    },
    "generate_bassline": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "root": {"type": int, "min": 0, "max": 127, "optional": True},
        "scale_type": {"type": str, "max_length": 32, "optional": True},
        "length": {"type": float, "min": 0.5, "max": 64, "optional": True},
    },
    # Audio clip editing
    "get_clip_gain": {
    ...
    "set_clip_gain": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "gain": {"type": float, "min": -70, "max": 24},
    },
    "set_clip_pitch": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "pitch": {"type": int, "min": -48, "max": 48},
    },
    "set_clip_warp_mode": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "warp_mode": {"type": int, "min": 0, "max": 6},
    },
    "get_clip_warp_info": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "add_warp_marker": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "beat_time": {"type": float, "min": 0, "max": 100000},
        "sample_time": {"type": float, "min": 0, "max": 1000000000, "optional": True},
    },
    "delete_warp_marker": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "set_clip_automation": {
        ...
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "parameter_index": {"type": int, "min": 0, "max": MAX_PARAMETER_INDEX},
        "points": {"type": list, "max_length": 10000},
    },
    "clear_clip_automation": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    # Group tracks
    "create_group_track": {
        "track_indices": {"type": list, "max_length": 100},
        "name": {"type": str, "max_length": 256, "optional": True},
    },
    "ungroup_tracks": {
        "group_track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX}
    },
    ...
    "apply_groove": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
        "groove_index": {"type": int, "min": 0, "max": 127},
    },
    "commit_groove": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "clip_index": {"type": int, "min": 0, "max": MAX_CLIP_INDEX},
    ...
    "search_browser": {
        "query": {"type": str, "max_length": 256},
        "category": {"type": str, "max_length": 32, "optional": True},
    },
    "get_browser_tree": {},
    "get_browser_items_at_path": {"path": {"type": list, "max_length": 20}},
    "load_browser_item": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "uri": {"type": str, "max_length": 2048},
    },
    "load_instrument_or_effect": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "uri": {"type": str, "max_length": 2048},
    ...
    "load_browser_item_to_return": {
        "return_index": {"type": int, "min": 0, "max": MAX_SEND_INDEX},
        "item_uri": {"type": str, "max_length": 2048},
    },
    # Rack chains
    "get_rack_chains": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
    ...
    "select_rack_chain": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "device_index": {"type": int, "min": 0, "max": MAX_DEVICE_INDEX},
        "chain_index": {"type": int, "min": 0, "max": 127},
    },
    "get_device_by_name": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "device_name": {"type": str, "max_length": 256},
    },
    "load_device_preset": {
        "track_index": {"type": int, "min": 0, "max": MAX_TRACK_INDEX},
        "device_index": {"type": int, "min": 0, "max": MAX_DEVICE_INDEX},
        "preset_uri": {"type": str, "max_length": 2048},
    },
}
    ...
    for param_name, rules in schema.items():
        ...
        if expected_type == int:
            if not isinstance(value, int) or isinstance(value, bool):
                raise HTTPException(
                    status_code=422, detail=f"Parameter {param_name} must be an integer"
                )
            if "min" in rules and value < rules["min"]:
                raise HTTPException(
                    status_code=422,
                    detail=f"Parameter {param_name} must be >= {rules['min']}",
                )
            if "max" in rules and value > rules["max"]:
                raise HTTPException(
                    status_code=422,
                    detail=f"Parameter {param_name} must be <= {rules['max']}",
                )
        elif expected_type == float:
            if not isinstance(value, (int, float)) or isinstance(value, bool):
                raise HTTPException(
                    status_code=422, detail=f"Parameter {param_name} must be a number"
                )
            value = float(value)
            if "min" in rules and value < rules["min"]:
                raise HTTPException(
            ...
            if not isinstance(value, bool):
                raise HTTPException(
                    status_code=422, detail=f"Parameter {param_name} must be a boolean"
                )
        elif expected_type == str:
            if not isinstance(value, str):
                raise HTTPException(
                    status_code=422, detail=f"Parameter {param_name} must be a string"
                )
            if "max_length" in rules and len(value) > rules["max_length"]:
                raise HTTPException(
                    status_code=422,
                    detail=f"Parameter {param_name} exceeds max length of {rules['max_length']}",
                )
            if "allowed_values" in rules and value not in rules["allowed_values"]:
                raise HTTPException(
                    status_code=422,
                    detail=f"Parameter {param_name} must be one of: {sorted(rules['allowed_values'])}",
                )
        elif expected_type == list:
            if not isinstance(value, list):
                raise HTTPException(
                    status_code=422, detail=f"Parameter {param_name} must be a list"
                )
            if "max_length" in rules and len(value) > rules["max_length"]:
                raise HTTPException(
                    status_code=422,
...
class GenericCommand(BaseModel):
    ...
    def validate_command(cls, v):
        if v not in ALLOWED_COMMANDS:
            raise ValueError(
                f"Unknown command: {v}. Use /api/commands to see available commands."
            )
        return v
...
# ============================================================================
# Audio-to-MIDI Transcription (Wonder feature)
# ============================================================================


class TranscribeAudioRequest(BaseModel):
    """Request body for audio transcription endpoint"""

    audio_data: str = Field(..., description="Base64-encoded audio data")
    input_format: str = Field("webm", description="Audio format: 'webm' or 'wav'")
    tempo_bpm: float = Field(
        120.0, ge=20, le=300, description="Tempo for beat conversion"
    )
    onset_threshold: float = Field(
        0.5, ge=0, le=1, description="Note onset detection threshold"
    )
    frame_threshold: float = Field(
        0.3, ge=0, le=1, description="Note frame detection threshold"
    )
    pitch_correction_strength: float = Field(
        0.7,
        ge=0,
        le=1,
        description="Pitch stabilization strength (0=off, 1=max correction)",
    )
...
def transcribe_audio(req: TranscribeAudioRequest):
    """
    Transcribe hummed/whistled audio to MIDI notes using basic-pitch.

    This endpoint is for Wonder's hum-to-MIDI feature. It converts audio
    (WebM or WAV format, base64-encoded) into MIDI note data that can be
    added to Ableton clips.

    Returns:
        - success: bool
        - notes: List of note objects with pitch, start_time, duration, velocity
        - note_count: Number of notes detected
        - suggested_clip_length: Recommended clip length in beats
        - tempo_bpm: The tempo used for conversion
        - error: Error message (if failed)
    """
    try:
        # Import the transcription module (lazy loaded to avoid slow startup)
        from audio_to_midi import transcribe_audio_base64

        result = transcribe_audio_base64(
            audio_base64=req.audio_data,
            ...
            pitch_correction_strength=req.pitch_correction_strength,
        )

        return result

    except ImportError as e:
        logger.error(f"Failed to import audio_to_midi module: {e}")
        return {
            "success": False,
            "error": "Audio transcription is not available. Make sure basic-pitch and pydub are installed.",
            "notes": [],
            "note_count": 0,
        }
    except Exception as e:
...
class LoadMidiNotesRequest(BaseModel):
    midi_id: str = Field(..., description="The midi_id from a previous transcription")


@app.post("/api/load_midi_notes")
def load_midi_notes(req: LoadMidiNotesRequest):
    """
    Load MIDI notes from a saved transcription file by midi_id.

    This is used when Gemini needs to get the actual notes from a midi_id
    reference (for token optimization - we only pass midi_id to Gemini,
    not the full notes array).

    Returns:
        - success: bool
        - notes: List of note objects (for add_notes_to_clip)
        - note_count: Number of notes
        - error: Error message (if failed)
    """
    try:
        from audio_to_midi import get_midi_file_path
        from mido import MidiFile

        midi_path = get_midi_file_path(req.midi_id)
        if not midi_path:
            return {
                "success": False,
                "error": f"MIDI file not found for midi_id: {req.midi_id}",
                "notes": [],
                "note_count": 0,
            }

        # Parse MIDI file to extract notes
            ...
            for msg in track:
                ...
                if msg.type == "note_on" and msg.velocity > 0:
                    ...
                    if msg.note in active_notes:
                        ...
                        duration = (current_time / ticks_per_beat) - start_time
                        notes.append(
                            {
                                "pitch": msg.note,
        ...
        logger.error(f"Failed to import mido: {e}")
        return {
            "success": False,
            "error": "MIDI parsing is not available. Make sure mido is installed: pip install mido",
            "notes": [],
            "note_count": 0,
        }
    except Exception as e:
        logger.error(f"Failed to load MIDI notes: {e}", exc_info=True)
        return {
            "success": False,
            "error": f"Failed to load MIDI notes: {str(e)}",
            "notes": [],
            "note_count": 0,
        }
