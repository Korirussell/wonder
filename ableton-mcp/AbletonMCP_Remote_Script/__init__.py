...
class AbletonMCP(ControlSurface):
            ...
            # Commands that modify Live's state should be scheduled on the main thread
                ...
                def main_thread_task():
                    try:
                        ...
                        elif command_type == "load_sample_by_path":
                            track_index = params.get("track_index", 0)
                            file_path = params.get("file_path", "")
                            device_index = params.get("device_index", 0)
                            pad_index = params.get("pad_index", None)
                            result = self._load_sample_by_path(track_index, file_path, device_index, pad_index)
                        elif command_type == "create_wonder_session":
                            bpm = params.get("bpm", 120)
                            tracks = params.get("tracks", [])
                            swing = params.get("swing", 0.15)
    ...
    # =========================================================================
    # Live 11+ Note API helpers (with Live 10 fallback)
    # =========================================================================

    def _write_notes_to_clip(self, clip, notes_tuples):
        """Write notes using Live 11+ API (add_new_notes) with fallback to set_notes.
        notes_tuples: list of (pitch, start_time, duration, velocity, mute)"""
        try:
            import Live
            notes_spec = []
            for pitch, start, duration, velocity, mute in notes_tuples:
                spec = Live.Clip.MidiNoteSpecification(
                    start_time=float(start),
                    duration=float(max(0.0625, duration)),
                    pitch=int(max(0, min(127, pitch))),
                    velocity=float(max(1, min(127, velocity))),
                    mute=bool(mute)
                )
                notes_spec.append(spec)
            clip.remove_notes_extended(0, 128, 0, clip.length)
            clip.add_new_notes(tuple(notes_spec))
        except (AttributeError, ImportError, TypeError):
            # Fall back to Live 10 legacy API
            clip.remove_notes(0, 0, clip.length, 128)
            clip.set_notes(tuple(notes_tuples))

    def _read_notes_from_clip(self, clip):
        """Read notes using Live 11+ API with fallback.
        Returns list of (pitch, start_time, duration, velocity, mute) tuples."""
        try:
            notes_ext = clip.get_notes_extended(0, 128, 0, clip.length)
            return [(int(n.pitch), float(n.start_time), float(n.duration),
                     int(n.velocity), bool(n.mute)) for n in notes_ext]
        except AttributeError:
            clip.select_all_notes()
            notes = list(clip.get_selected_notes())
            clip.deselect_all_notes()
            return notes

    def _humanize_clip_timing(self, track_index, clip_index, amount):
        """Add random timing variation to notes (Live 11+ compatible)"""
        try:
            import random
            ...
            notes_data = self._read_notes_from_clip(clip)
            humanized = []
            for pitch, start, duration, velocity, mute in notes_data:
                offset = (random.random() - 0.5) * 2 * amount
                new_start = max(0.0, start + offset)
                humanized.append((pitch, new_start, duration, velocity, mute))

            self._write_notes_to_clip(clip, humanized)
            return {"humanized": True, "note_count": len(humanized), "amount": amount}
        except Exception as e:
            self.log_message("Error humanizing clip timing: " + str(e))
            raise

    def _humanize_clip_velocity(self, track_index, clip_index, amount):
        """Add random velocity variation to notes (Live 11+ compatible)"""
        try:
            import random
            ...
            for pitch, start, duration, velocity, mute in notes_data:
                ...
                humanized.append((pitch, start, duration, new_velocity, mute))
    ...
    def _generate_drum_pattern(self, track_index, clip_index, style, length):
        """Generate a humanized drum pattern (Live 11+ compatible).
        Styles: basic, house, hiphop, dnb, lofi, trap, jazz, afrobeats, experimental"""
        try:
            import random
            ...
            CLAP = 39; RIMSHOT = 37; TOMLO = 45; TOMHI = 50; CRASH = 49; RIDE = 51

            def jv(base, spread=8):
                """Jittered velocity — adds humanization to every note"""
                return max(1, min(127, base + random.randint(-spread, spread)))

            def jt(base, spread=0.01):
                """Jittered timing"""
                return max(0.0, base + (random.random() - 0.5) * 2 * spread)
            ...
            if style == "basic":
                for beat in range(beats):
                    if beat % 2 == 0:
                        notes.append((KICK,      jt(float(beat)), 0.25, jv(100), False))
                    if beat % 2 == 1:
                        notes.append((SNARE,     jt(float(beat)), 0.25, jv(95), False))
                    for e in range(2):
                        notes.append((CLOSED_HH, jt(beat + e*0.5), 0.1, jv(70, 12), False))

            elif style == "house":
                for beat in range(beats):
                    notes.append((KICK,     jt(float(beat)), 0.2, jv(112, 5), False))
                    if beat % 2 == 1:
                        notes.append((CLAP,  jt(float(beat), 0.005), 0.15, jv(100, 6), False))
                    notes.append((OPEN_HH,  jt(beat + 0.5, 0.015), 0.1, jv(82, 10), False))
                    # 16th hat sprinkle
                    for s in [0.25, 0.75]:
                        if random.random() > 0.45:
                            notes.append((CLOSED_HH, jt(beat + s), 0.08, jv(55, 15), False))

            elif style == "hiphop":
                for beat in range(beats):
                    if beat % 4 == 0:
                        notes.append((KICK,  jt(float(beat)), 0.25, jv(110, 5), False))
                    if beat % 4 == 2:
                        notes.append((KICK,  jt(beat + 0.75, 0.02), 0.2, jv(88, 8), False))
                    if beat % 2 == 1:
                        notes.append((SNARE, jt(float(beat), 0.008), 0.2, jv(100, 7), False))
                        # Ghost snare
                        if random.random() > 0.5:
                            notes.append((SNARE, jt(beat - 0.25, 0.02), 0.1, jv(35, 10), False))
                    for e in range(2):
                        vel = jv(78 if e == 0 else 52, 12)
                        notes.append((CLOSED_HH, jt(beat + e*0.5, 0.015), 0.08, vel, False))
                ...
                for beat in range(beats):
                    if beat % 4 == 0:
                        notes.append((KICK,  jt(float(beat)), 0.2, jv(112, 5), False))
                    if beat % 4 == 2:
                        notes.append((KICK,  jt(beat + 0.5, 0.01), 0.2, jv(100, 8), False))
                    if beat % 2 == 1:
                        notes.append((SNARE, jt(float(beat), 0.006), 0.15, jv(110, 5), False))
                    for s in range(4):
                        notes.append((CLOSED_HH, jt(beat + s*0.25, 0.01), 0.07,
                                       jv(62 + (12 if s % 2 == 0 else 0), 10), False))

            elif style == "lofi":
                # Swung lo-fi hip-hop — lazy kick, cracking snare, ghost notes
                for beat in range(beats):
                    if beat % 4 == 0:
                        notes.append((KICK,  jt(float(beat), 0.02), 0.3, jv(105, 8), False))
                    if beat % 4 == 2:
                        if random.random() > 0.3:
                            notes.append((KICK, jt(beat + 0.75, 0.03), 0.2, jv(80, 12), False))
                    if beat % 2 == 1:
                        notes.append((SNARE,   jt(float(beat), 0.015), 0.2, jv(95, 10), False))
                        notes.append((RIMSHOT, jt(float(beat), 0.02),  0.1, jv(60, 15), False))
                    # Swung hi-hat (8th notes with swing feel)
                    notes.append((CLOSED_HH, jt(float(beat), 0.01), 0.08, jv(72, 14), False))
                    notes.append((CLOSED_HH, jt(beat + 0.5, 0.025), 0.08, jv(48, 16), False))
                    # Occasional open hat
                    if random.random() > 0.7:
                        notes.append((OPEN_HH, jt(beat + 0.5, 0.02), 0.15, jv(55, 12), False))
                    # Ghost snares
                    for ghost_pos in [0.25, 0.75]:
                        if random.random() > 0.65:
                            notes.append((SNARE, jt(beat + ghost_pos, 0.025), 0.08,
                                           jv(28, 8), False))

            elif style == "trap":
                for beat in range(beats):
                    if beat % 4 == 0:
                        notes.append((KICK, jt(float(beat)), 0.25, jv(115, 5), False))
                    if beat % 4 in [1, 2]:
                        if random.random() > 0.4:
                            notes.append((KICK, jt(float(beat), 0.01), 0.2, jv(90, 10), False))
                    if beat % 2 == 1:
                        notes.append((SNARE, jt(float(beat), 0.005), 0.2, jv(105, 6), False))
                    # Trap hi-hat rolls — 16ths and 32nds
                    for s in range(4):
                        vel = jv(90 if s == 0 else 60, 15)
                        notes.append((CLOSED_HH, jt(beat + s*0.25, 0.008), 0.06, vel, False))
                    # Random 32nd hi-hat bursts
                    if random.random() > 0.6:
                        burst_start = beat + random.choice([0.5, 0.75, 1.0])
                        for micro in range(random.randint(2, 4)):
                            notes.append((CLOSED_HH, jt(burst_start + micro*0.125, 0.01),
                                           0.05, jv(50, 20), False))

            elif style == "jazz":
                for beat in range(beats):
                    # Ride cymbal on every beat + swing 8ths
                    notes.append((RIDE, jt(float(beat), 0.008), 0.12, jv(80, 12), False))
                    # Swing 8th on ride
                    notes.append((RIDE, jt(beat + 0.67, 0.015), 0.1, jv(65, 15), False))
                    # Hi-hat on 2 and 4
                    if beat % 2 == 1:
                        notes.append((CLOSED_HH, jt(float(beat), 0.005), 0.05,
                                       jv(90, 8), False))
                    # Sparse kick and snare (comping feel)
                    if random.random() > 0.6:
                        notes.append((KICK,  jt(float(beat), 0.02), 0.2, jv(75, 15), False))
                    if random.random() > 0.55:
                        notes.append((SNARE, jt(beat + random.choice([0, 0.33, 0.67]), 0.02),
                                       0.15, jv(70, 18), False))
                    # Occasional crash
                    if beat == 0 and random.random() > 0.7:
                        notes.append((CRASH, jt(0.0), 0.5, jv(85, 10), False))

            elif style == "afrobeats":
                for beat in range(beats):
                    # Kick on 1 and the "and" of 2
                    if beat % 4 == 0:
                        notes.append((KICK, jt(float(beat)), 0.2, jv(110, 6), False))
                    if beat % 4 == 2:
                        notes.append((KICK, jt(beat + 0.5, 0.015), 0.2, jv(95, 8), False))
                    # Clap on 2 and 4
                    if beat % 2 == 1:
                        notes.append((CLAP, jt(float(beat), 0.008), 0.15, jv(100, 7), False))
                    # Shaker-style 16th hi-hats
                    for s in range(4):
                        accent = jv(82 if s % 2 == 0 else 58, 12)
                        notes.append((CLOSED_HH, jt(beat + s*0.25, 0.01), 0.07, accent, False))
                    # Sporadic open hi-hat
                    if random.random() > 0.6:
                        notes.append((OPEN_HH, jt(beat + 0.75, 0.02), 0.12, jv(70, 12), False))

            else:  # experimental / random
                for beat in range(beats):
                    if random.random() > 0.3:
                        notes.append((KICK,  jt(beat + random.choice([0, 0.5])), 0.2,
                                       jv(95, 15), False))
                    if random.random() > 0.5:
                        notes.append((SNARE, jt(beat + random.choice([0, 0.25, 0.5])), 0.15,
                                       jv(85, 18), False))
                    if random.random() > 0.2:
                        notes.append((CLOSED_HH, jt(beat + random.random()*0.5), 0.08,
                                       jv(65, 20), False))

            self._write_notes_to_clip(clip, notes)
            return {"generated": True, "style": style, "length": length, "note_count": len(notes)}
        except Exception as e:
    ...
    def _generate_bassline(self, track_index, clip_index, root, scale_type, length):
        """Generate a humanized bassline (Live 11+ compatible).
        scale_type: major, minor, dorian, phrygian, pentatonic_minor, blues, lydian, mixolydian"""
        try:
            import random
            ...
            for beat in range(beats):
                # Anchor root on every bar downbeat
                if beat % 4 == 0:
                    vel = random.randint(95, 110)
                    notes.append((root, float(beat), 0.45, vel, False))
                else:
                    interval = random.choice(scale)
                    note = root + interval
                    if random.random() > 0.75:
                        note += 12   # occasional octave jump
                    dur = random.choice([0.2, 0.25, 0.4, 0.5])
                    vel = random.randint(75, 105)
                    t = beat + (random.random() - 0.5) * 0.02  # slight timing humanization
                    notes.append((note, max(0.0, t), dur, vel, False))

                # 8th-note movement (~50% chance)
                if random.random() > 0.45:
                    interval = random.choice(scale[:4])  # stay close to root
                    note = root + interval
                    t = beat + 0.5 + (random.random() - 0.5) * 0.015
                    vel = random.randint(65, 88)
                    notes.append((note, max(0.0, t), 0.2, vel, False))

                # Occasional 16th-note pickup (adds groove)
                if random.random() > 0.7:
                    interval = random.choice(scale[:3])
                    note = root + interval
                    t = beat + 0.75 + (random.random() - 0.5) * 0.01
                    notes.append((note, max(0.0, t), 0.15, random.randint(60, 80), False))

            self._write_notes_to_clip(clip, notes)
            return {
                "generated": True, "root": root,
                "scale_type": scale_type, "length": length,
                "note_count": len(notes)
            }
        except Exception as e:
    ...
    # =========================================================================
    # Sample loading by file path (Wonder ElevenLabs pipeline)
    # =========================================================================

    def _find_sample_in_browser(self, browser, filename):
        """Search browser for a sample file by exact filename."""
        name_lower = filename.lower()
        targets = []
        for attr in ("user_library", "sounds", "drums"):
            if hasattr(browser, attr):
                targets.append(getattr(browser, attr))
        ...
        for target in targets:
            result = _search(target)
            if result:
                return result
        return None

    def _load_sample_by_path(self, track_index, file_path, device_index=0, pad_index=None):
        """Copy a .wav file to the Wonder User Library folder and load it onto a track.
        Supports: loading into a Simpler, a specific Drum Rack pad, or bare onto the track.

        Parameters:
        - track_index: target track
        - file_path: absolute path to the .wav / .aif file
        - device_index: which device on the track to target (default 0)
        - pad_index: if set, select this Drum Rack pad before loading (0–127)
        """
        try:
            import os
            import shutil
            ...
            # Copy to User Library so browser can find it
            dest_dir = os.path.expanduser(
                "~/Music/Ableton/User Library/Samples/Imported/Wonder/")
            os.makedirs(dest_dir, exist_ok=True)
            filename = os.path.basename(file_path)
            dest = os.path.join(dest_dir, filename)
            if not os.path.exists(dest) or os.path.getmtime(file_path) > os.path.getmtime(dest):
                shutil.copy2(file_path, dest)

            # Try to locate in browser
            app = self.application()
            item = self._find_sample_in_browser(app.browser, filename)

            if not item:
                return {
                    "status": "copied_not_indexed",
                    "file": filename,
                    "dest": dest,
                    "note": (
                        "File copied to User Library but not yet indexed by Ableton browser. "
                        "Wait ~1s and call load_sample_by_path again — Ableton auto-scans on file change."
                    )
                }

            # Select the track (and optionally a Drum Rack pad)
            self._song.view.selected_track = track
                ...
                if device_index < len(devices):
                    ...
                    if hasattr(device, "drum_pads"):
                        ...
                        if pad_index < len(pads):
                            try:
                                device.view.selected_drum_pad = pads[pad_index]
                            except Exception:
                                pass
            ...
            self.log_message("Error loading sample by path: " + str(e))
            raise

    # =========================================================================
    # Wonder composite session builder
    # =========================================================================

    def _create_wonder_session(self, bpm, tracks, swing=0.15, key_root=0, scale="minor"):
        """Build a complete Wonder session in one command.

        Parameters:
        - bpm: float
        - swing: 0.0-1.0 (default 0.15)
        - key_root: MIDI pitch of the tonic (default 0 = C)
        - scale: scale name for bassline generation
        - tracks: list of track definitions, each a dict with:
            {
              "type": "midi" | "audio",
              "name": str,
              "volume": float (0-1, default 0.85),
              "color": int (optional),
              "clip_length": float (bars, default 4),
              "clip_name": str (optional),
              "pattern": str (drum style, triggers generate_drum_pattern),
              "bassline": bool (triggers generate_bassline with key_root + scale),
              "notes": list of {pitch, start_time, duration, velocity, mute} (raw MIDI),
              "instrument_uri": str (optional browser URI to load instrument),
            }

        Returns dict with created track indices and summary.
        """
        try:
            self._song.tempo = float(bpm)
            self._song.swing_amount = float(swing)

            created = []

            for track_def in tracks:
                track_type = track_def.get("type", "midi")
                base_idx = len(self._song.tracks)

                if track_type == "audio":
                    self._song.create_audio_track(base_idx)
                else:
                    self._song.create_midi_track(base_idx)

                track = self._song.tracks[base_idx]
                track.name = track_def.get("name", "Wonder Track {}".format(base_idx))

                volume = track_def.get("volume", 0.85)
                ...
                clip_length = float(track_def.get("clip_length", 4))
                clip_name   = track_def.get("clip_name", track.name)
                pattern     = track_def.get("pattern")
                bassline    = track_def.get("bassline", False)
                raw_notes   = track_def.get("notes")
                instr_uri   = track_def.get("instrument_uri")

                # Load instrument if URI provided
                if instr_uri:
                    try:
                        app = self.application()
                        item = self._find_browser_item_by_uri(app.browser, instr_uri)
                        if item:
                            self._song.view.selected_track = track
                            app.browser.load_item(item)
                    except Exception as instr_err:
                        self.log_message("Warning: could not load instrument URI: " + str(instr_err))
                ...
                if pattern:
                    ...
                    result = self._generate_drum_pattern(base_idx, 0, pattern, clip_length)
                    note_count = result.get("note_count", 0)
                    clip_slot.clip.name = clip_name
                    ...
                    if not clip_slot.has_clip:
                        clip_slot.create_clip(clip_length)
                    result = self._generate_bassline(base_idx, 0, key_root, scale, clip_length)
                    note_count = result.get("note_count", 0)
                    clip_slot.clip.name = clip_name
                    ...
                    clip_slot.clip.name = clip_name
                    note_tuples = [
                        (n["pitch"], n["start_time"], n["duration"],
                         n.get("velocity", 100), n.get("mute", False))
                        for n in raw_notes
                    ]
                    self._write_notes_to_clip(clip_slot.clip, note_tuples)
            ...
            return {
                ...
                "swing":   swing,
                "tracks":  created,
                "track_count": len(created),
            }
        except Exception as e:
            self.log_message("Error creating Wonder session: " + str(e))
            raise
